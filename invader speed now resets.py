...
invader_speed_increase= 20 #this increase their speed when one is shot away
invader_step_time_original = invader_step_time


def reset_invaders():
    global invaders, invader_step_time
    invader_step_time = invader_step_time_original
    ...
    row_images = [invader_img_row1, invader_img_row2, invader_img_row3]
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            image = row_images[row]
            invaders.append({"rect": rect, "image": image})

invaders = []
reset_invaders()

def game_over_screen(score): # this defines the game over screen as  function
    Large_Text = pygame.font.SysFont(None, 80)
    Small_Text = pygame.font.SysFont(None, 40)

    game_over_text = Large_Text.render("GAME OVER", True, (255, 0, 0))
    score_text = Small_Text.render(f"Final Score: {score}", True, (255, 255, 255))
    message_text = Small_Text.render("Press any key to exit", True, (255, 255, 255))
    ...
    while True:
        ...
        screen.blit(message_text, (WIDTH//2 - message_text.get_width()//2, 400))
...
while running:
    ...
    #i've added this for debugging quicker so i don't have to run out of lives 
    if keys[pygame.K_t]:
...
# Shooting bullets with space bar (max 2 bullets, 1s delay)
    ...
    for bullet in bullets[:]:
        ...
        for inv in invaders[:]:
            if bullet.colliderect(inv["rect"]):
                ...
                bullets.remove(bullet)
                score += 10   
                invader_step_time = max(160, invader_step_time - invader_speed_increase)  # Speed up invaders after each kill
    ...
    for eb in enemy_bullets[:]:
        ...
        if eb.colliderect(player_rect) and player_alive:
            ...
            if player_lives <= 0: # if the player score reaches 0 it will run the game_over_screen function
                game_over_screen(score)
    ...
    speed_text = font.render(f"Speed delay: {invader_step_time} ms", True, (0, 255, 0))
